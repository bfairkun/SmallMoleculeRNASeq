...
Colors <- sample.list %>%
  dplyr::select(old.sample.name, color) %>%
  right_join(data.frame(old.sample.name = as.character(colnames(ExpressedGeneCounts)))) %>%
  deframe()
...
ExpressedGeneCounts %>%
  cpm(log=T) %>%
  cor(method="pearson") %>%
  heatmap.2(trace="none", RowSideColors = Colors, ColSideColors = Colors)
```
...
leafcutter.ds.annotated %>%
  ...
  geom_vline(
    data = . %>%
      group_by(UpstreamOfGAGT, treatment) %>%
      summarise(med=median(logef, na.rm=T)),
    aes(xintercept=med, color=treatment),
    linetype='dashed'
  ) +
...
##### Using OMIM annotations for searching potential targets
...
DE_and_DS_OMIM_Map %>%
  ...
  # filter(UpstreamOfDonor2BaseSeq == "GA" ) %>%
  select(Gene, `Preferred Title; symbol`, UpstreamOfDonor2BaseSeq, psi_treatment, intron, logFC)  %>%
  write_tsv("../output/FibroblastOMIM_GOF_Downregulated_AndCrypticSpliced_GeneList.tsv")
  

```

Now let's write out some info to help me plot stuff in a useful way with my pygenometracks scripts. More specifically, write out a bash script which I will then run as follows in the next code chunk

```{r, eval=F}

genes.to.plot <- DE_and_DS_OMIM_Map %>%
  distinct(treatment, Gene, MIM_phenotype, .keep_all = T) %>%
  filter(treatment == "SM2") %>%
  filter(ModeInheritence=="GainOfFunction" & IsDownregulate & IsCrypticSplicingSig)

# Method1 for selecting introns to plot sashimi
leafcutter.ds.annotated %>%
  distinct(intron, cluster) %>%
  filter(cluster %in% genes.to.plot$cluster) %>%
  separate(intron, into=c("chrom", "start", "stop", "cluster2"), sep=":", remove=F, convert=T) %>%
  select(chrom, start, stop, intron) %>%
  arrange(chrom, start, stop) %>%
  write_tsv("../code/scratch/ToPlot.introns.bed")

# Method2 for selecting introns to plot sashimi
...
```{r, eval=F}
dir.create("../code/scratch/20220927_PlotExamples")

# RNF170 junc: "chr8:42874006:42887728"
# HTT 5'ss: chr4:3,213,737
      ...
      mutate(gene_ids=str_replace(gene_ids, "\\..+$", "")) %>%
        full_join(
          (DE.results %>%
             mutate(gene_ids=str_replace(EnsemblID, "\\..+$", ""))), 
        by=c("treatment","gene_ids")) %>%
  filter(str_detect(intron, "clu_11248_-") | str_detect(intron, "clu_6379_+")) %>% 
  filter(UpstreamOfDonor2BaseSeq == "GA" | IntronType=="Annotated" | IntronType == "Alt 3'ss" ) %>%
  distinct(intron, .keep_all=T) %>%
  separate(intron, into=c("chrom", "start", "end", "clu"), convert=T, sep=":") %>%
  dplyr::select(chrom, start, end) %>%
  arrange(chrom, start, end) %>%
  write_tsv("../code/scratch/20220927_PlotExamples/.Juncs.bed", col_names=T)
...
leafcutter.ds.annotated %>%
      ...
      mutate(gene_ids=str_replace(gene_ids, "\\..+$", "")) %>%
        ...
        by=c("treatment","gene_ids")) %>%
  filter(str_detect(intron, "clu_11248_-") | str_detect(intron, "clu_6379_+")) %>% 
  distinct(gene_ids, treatment, .keep_all=T)

#Write splice donors and vertical line track
fileConn<-file("../code/scratch/20220927_PlotExamples/.SpliceDonors.bed")
writeLines(c("chr8\t42887728\t42887729", "chr4\t3213737\t3213738"), con=fileConn)
close(fileConn)

fileConn<-file("../code/scratch/20220927_PlotExamples/.SpliceDonors.ini")
writeLines(c("[vlines]","type = vlines", "file = scratch/20220927_PlotExamples/.SpliceDonors.bed"), con=fileConn)
close(fileConn)


read_tsv("../code/bigwigs/BigwigList.groups.tsv", col_names=c("GroupName", "Color", "PSI", "Supergroup")) %>%
  filter(str_detect(GroupName, "_u$")) %>%
  filter(str_detect(GroupName, "Fibroblast")) %>%
  mutate(Supergroup = "FibroblastExp") %>%
  write_tsv("../code/scratch/20220927_PlotExamples/.groups.tsv")
...
# python helper plotting script RNF170
python scripts/GenometracksByGenotype/AggregateBigwigsForPlotting.py --GroupSettingsFile scratch/20220927_PlotExamples/.groups.tsv --BigwigList bigwigs/BigwigList.tsv --Normalization WholeGenome --Region chr8:42847637-42898605 --BigwigListType KeyFile --FilterJuncsByBed scratch/20220927_PlotExamples/.Juncs.bed --OutputPrefix scratch/20220927_PlotExamples/. -vv --TracksTemplate scripts/GenometracksByGenotype/tracks_templates/GeneralPurposeColoredByGenotypeWithSupergroups.ini --Bed12GenesToIni scripts/GenometracksByGenotype/PremadeTracks/gencode.v26.FromGTEx.genes.bed12.gz

# pygenomtracks plotting
pyGenomeTracks --tracks <(cat scratch/20220927_PlotExamples/.tracks.ini scratch/20220927_PlotExamples/.SpliceDonors.ini) --out scratch/20220927_PlotExamples/RNF170.pdf --region chr8:42847637-42898605 --trackLabelFraction 0.15 --title RNF170

# python helper plotting script HTT
python scripts/GenometracksByGenotype/AggregateBigwigsForPlotting.py --GroupSettingsFile scratch/20220927_PlotExamples/.groups.tsv --BigwigList bigwigs/BigwigList.tsv --Normalization WholeGenome --Region chr4:3,211,313-3,216,142 --BigwigListType KeyFile --FilterJuncsByBed scratch/20220927_PlotExamples/.Juncs.bed --OutputPrefix scratch/20220927_PlotExamples/. -vv --TracksTemplate scripts/GenometracksByGenotype/tracks_templates/GeneralPurposeColoredByGenotypeWithSupergroups.ini --Bed12GenesToIni scripts/GenometracksByGenotype/PremadeTracks/gencode.v26.FromGTEx.genes.bed12.gz

# pygenomtracks plotting
pyGenomeTracks --tracks <(cat scratch/20220927_PlotExamples/.tracks.ini scratch/20220927_PlotExamples/.SpliceDonors.ini) --out scratch/20220927_PlotExamples/HTT.pdf --region chr4:3,211,313-3,216,142 --trackLabelFraction 0.15 --title HTT
