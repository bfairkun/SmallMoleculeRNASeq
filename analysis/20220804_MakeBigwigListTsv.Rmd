...
chRNA.seq.experiment <- read_tsv("../data/chRNA_sampleLibraryKey.tsv") %>%
  ...
  select(OldSampleID, Group_label, SampleID)
...
FibroblastExp <- read_tsv("../code/config/samples.tsv") %>%
  ...
  mutate(SampleID =  paste(Treatment, dose.nM, CellType, LibType, Replicate,sep = "_")) %>%
  mutate(Group_label = paste(Treatment, dose.nM, CellType, LibType, sep = "_")) %>%
  select(OldSampleID=sample, Group_label, SampleID)
...
```{r}
bigwigs.df %>%
  select(SampleID, BigwigFilepath, Group_label, Strand ) %>%
...
bigwigs.df %>%
  ...
  arrange(LibType, CellType, LibType, Treatment, dose.nM, Group_label) %>%
  select(Group_label, Color, BedgzFile) %>%
